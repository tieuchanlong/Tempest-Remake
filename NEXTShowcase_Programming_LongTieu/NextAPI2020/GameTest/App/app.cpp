...
{	
	...
	void DrawSquare(float center_x, float center_y, float size, float r, float g, float b) 
	{
#if APP_USE_VIRTUAL_RES		
		APP_VIRTUAL_TO_NATIVE_COORDS(center_x, center_y);
#endif
		glBegin(GL_QUADS);

		//draw first triangle
		glColor3f(r, g, b);
		glVertex2f(center_x - size/APP_VIRTUAL_WIDTH, center_y - size/APP_VIRTUAL_HEIGHT);
		glVertex2f(center_x + size / APP_VIRTUAL_WIDTH, center_y - size / APP_VIRTUAL_HEIGHT);
		glVertex2f(center_x + size / APP_VIRTUAL_WIDTH, center_y + size / APP_VIRTUAL_HEIGHT);
		glVertex2f(center_x - size / APP_VIRTUAL_WIDTH, center_y + size / APP_VIRTUAL_HEIGHT);
	...
	void DrawTriangle(float center_x, float center_y, float size, float r, float g, float b)
	{
#if APP_USE_VIRTUAL_RES		
		APP_VIRTUAL_TO_NATIVE_COORDS(center_x, center_y);
#endif
		...
		//draw first triangle
		glColor3f(r, g, b);
		glVertex2f(center_x + size * cos(30 * PI / 180) / APP_VIRTUAL_WIDTH, center_y - size * sin(30 * PI / 180) / APP_VIRTUAL_HEIGHT);
		glVertex2f(center_x, center_y + size / APP_VIRTUAL_HEIGHT);
		glVertex2f(center_x - size * cos(30 * PI / 180) / APP_VIRTUAL_WIDTH, center_y - size * sin(30 * PI / 180) / APP_VIRTUAL_HEIGHT);
